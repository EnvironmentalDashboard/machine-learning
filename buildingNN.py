		...
		for meter in cur.fetchall():
			...
			for data_point in cur.fetchall():
				if random.randint(0, 100) > 50:
					test_set.append([(building[1] + ' ' + meter[1]), data_point[0], datetime.datetime.fromtimestamp(int(data_point[1])).strftime('%m-%d'), building[2], building[3], building[4], building[5]])
				else:
					training_set.append([(building[1] + ' ' + meter[1]), data_point[0], datetime.datetime.fromtimestamp(int(data_point[1])).strftime('%m-%d'), building[2], building[3], building[4], building[5]])
	db.close()
